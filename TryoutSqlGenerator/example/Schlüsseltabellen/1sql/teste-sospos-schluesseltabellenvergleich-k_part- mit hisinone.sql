...
FROM dblink('FGLconnection202412', 'SELECT id, uniquename, hiskey_id, defaulttext FROM hisinone.k_examination_type') AS t2(id text, uniquename text, hiskey_id text, defaulttext text) 
FULL OUTER JOIN dblink('FGLconnectionInsertReference', 'SELECT id, uniquename, hiskey_id, defaulttext FROM hisinone.k_examination_type') AS t1(id text, uniquename text, hiskey_id text, defaulttext text) on t2.uniquename = t1.uniquename;
...
select 
CASE
  WHEN COALESCE(t1.uniquename,'') != COALESCE(part,'') then '!'
 ELSE
  ''
 END AS abweichung,
 part, dtxt, t1.uniquename, t1.defaulttext, t1.hiskey_id
 
FROM k_part
FULL OUTER JOIN dblink('FGLconnectionInsertReference', 'SELECT id, uniquename, hiskey_id, defaulttext FROM hisinone.k_examination_type') AS t1(id text, uniquename text, hiskey_id text, defaulttext text) on part = t1.uniquename;
