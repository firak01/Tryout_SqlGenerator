...
FROM dblink('FGLconnection202412', 'SELECT id, uniquename, hiskey_id, defaulttext FROM hisinone.k_compulsory') AS t2(id text, uniquename text, hiskey_id text, defaulttext text) 
FULL OUTER JOIN dblink('FGLconnectionInsertReference', 'SELECT id, uniquename, hiskey_id, defaulttext FROM hisinone.k_compulsory') AS t1(id text, uniquename text, hiskey_id text, defaulttext text) on t2.uniquename = t1.uniquename;
...
FROM k_ppflic
FULL OUTER JOIN dblink('FGLconnectionInsertReference', 'SELECT id, uniquename, hiskey_id, defaulttext FROM hisinone.k_compulsory') AS t1(id text, uniquename text, hiskey_id text, defaulttext text) on ppflicht = t1.uniquename;
