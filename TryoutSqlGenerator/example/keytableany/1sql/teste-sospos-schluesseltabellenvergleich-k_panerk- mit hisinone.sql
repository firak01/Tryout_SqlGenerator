...
--SELECT public.dblink_connect('FGLconnectionInsertReferenz', 'dbname=h1_tubaf_202312_20250213 host=127.0.0.1 port=5432 user=postgres password=postgres');
...
FROM dblink('FGLconnection202412', 'SELECT id, uniquename, hiskey_id, defaulttext FROM hisinone.k_accreditationtype') AS t2(id text, uniquename text, hiskey_id text, defaulttext text) 
FULL OUTER JOIN dblink('FGLconnectionInsertReference', 'SELECT id, uniquename, hiskey_id, defaulttext FROM hisinone.k_accreditationtype') AS t1(id text, uniquename text, hiskey_id text, defaulttext text) on t2.uniquename = t1.uniquename;

--#############
-- Vergleich SOSPOS mit aktuellem HIO
select 
CASE
  WHEN COALESCE(t1.uniquename,'') != COALESCE(panerk,'') then '!'
 ELSE
  ''
 END AS abweichung,
 panerk, dtxt, t1.uniquename, t1.defaulttext, t1.hiskey_id
 
FROM k_panerk
FULL OUTER JOIN dblink('FGLconnectionInsertReference', 'SELECT id, uniquename, hiskey_id, defaulttext FROM hisinone.k_accreditationtype') AS t1(id text, uniquename text, hiskey_id text, defaulttext text) on panerk = t1.uniquename;
