...
public abstract class AbstractKeyPressThreadSqlUtilZZZ extends AbstractKeyPressThreadZZZ implements IKeyPressThreadSqlUtilConstantZZZ{
	public AbstractKeyPressThreadSqlUtilZZZ(IConsoleZZZ objConsole) {
    	super(objConsole);
    }
    public AbstractKeyPressThreadSqlUtilZZZ(IConsoleZZZ objConsole, long lSleepTime) {
	...
	protected void questionNumericKey(HashMapZZZ hmVariable) throws ExceptionZZZ {
    	...
    	if(!this.isCurrentInputFinished()) {
    		...
    		if(StringZZZ.equalsIgnoreCase(sInput, IKeyPressConstantZZZ.cKeyCancel)){					                		
        		...
        		if(hmVariable!=null) hmVariable.put(KeyPressThreadSqlUtilZZZ.sINPUT_KEY_NUMERIC, sInput);					
        	}	
    	}
	}
    	...
    	if(!this.isCurrentInputFinished()) {
    		...
    		if(!bCharacterPoolContainsNumericOnly) {
        		...
        		if(StringZZZ.equalsIgnoreCase(sInput, IKeyPressConstantZZZ.cKeyCancel)){
        			...
        			this.isCurrentInputValid(true);	
                	if(hmVariable!=null) hmVariable.put(KeyPressThreadSqlUtilZZZ.sINPUT_FLAG_CHARACTER_NUMERIC, BooleanZZZ.stringToBoolean(sInput));
            	}else {
            		this.isCurrentInputValid(true);	
            		if(hmVariable!=null) hmVariable.put(KeyPressThreadSqlUtilZZZ.sINPUT_FLAG_CHARACTER_NUMERIC, BooleanZZZ.stringToBoolean(sInput));
            		if(hmVariable!=null) hmVariable.put(KeyPressThreadSqlUtilZZZ.sINPUT_FLAG_USE_STRATEGY_CHARACTERPOOL, BooleanZZZ.stringToBoolean(sInput));
            	}	        		
    		}
    	}
	}
	...
	protected void questionUseStrategy_CaseChange(HashMapZZZ hmVariable, String sCharacterPool) throws ExceptionZZZ {
    	...
    	if(!this.isCurrentInputFinished()) {
    		...
    		if(StringZZZ.equalsIgnoreCase(sInput, IKeyPressConstantZZZ.cKeyCancel)){
    			...
    			this.isCurrentInputValid(true);	
    			if(hmVariable!=null) hmVariable.put(KeyPressThreadSqlUtilZZZ.sINPUT_FLAG_USE_STRATEGY_CASECHANGE, BooleanZZZ.stringToBoolean(sInput));
        	}else {
        		this.isCurrentInputValid(true);	            		
        		if(hmVariable!=null) hmVariable.put(KeyPressThreadSqlUtilZZZ.sINPUT_FLAG_USE_STRATEGY_CASECHANGE, BooleanZZZ.stringToBoolean(sInput));
